...
| `xinix_hippo_history` | Per favoriet: gemeten tellingen uit 10 jaar dagkoersen (+50%/14d-treffers per kenmerk-bucket, kalibratie-histogram) |
| `xinix_hippo_scores` | Hippos-ranglijst: gekalibreerde kans op +50% binnen 14 dagen per favoriet, factoren, laatste melding |
| `xinix_hippo_calibration` | Eén rij: gepoolde basiskans, lifts per kenmerk en kalibratie (model vs. werkelijkheid) van de laatste run |
