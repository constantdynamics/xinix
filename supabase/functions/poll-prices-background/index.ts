...
Deno.serve(
  runBackground("poll-prices", async () => {
    ...
    for (const { ticker } of tickers) {
      try {
        ...
        const valid = bars.filter(
          (b): b is YahooBar & { close: number } => b.close !== null
        );
        ...
        if (summary.pct_change_1d >= 30 && volRatio >= 3) {
          const id = await insertSignal(supabase, {
            ...
            severity: "red",
            title: `${ticker} +${summary.pct_change_1d.toFixed(
              1
            )}% met volume ${volRatio.toFixed(1)}×`,
            detail: `Koers $${last.close.toFixed(
              2
            )} (was $${prev?.close.toFixed(2)}). Vrijwel zeker net-event.`,
          ...
          const id = await insertSignal(supabase, {
            ...
            detail: `Koers $${last.close.toFixed(
              2
            )}. Volume ratio ${volRatio.toFixed(1)}×.`,
            payload: { pct: summary.pct_change_1d, volume_ratio: volRatio },
            expires_at: expires7,
            dedup_key: `price_spike_up:${ticker}:${today}`,
          });
