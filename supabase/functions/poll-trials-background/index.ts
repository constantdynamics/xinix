...
Deno.serve(
  runBackground("poll-trials", async () => {
        ...
        for (const s of studies) {
          ...
          if (
            ...
            const sigId = await insertSignal(supabase, {
              ...
              severity: "orange",
              title: `${ticker} trial COMPLETED — ${id.briefTitle?.slice(
                0,
                60
              )}`,
              detail: `${phase ?? ""} ${
                id.nctId
              } status → COMPLETED. Topline kan binnen weken komen.`,
    ...
    return {
      ok: errors.length === 0,
      message:
        `${trialsTracked} trials, ${catalystsAdded} catalysts, ${signalsInserted} signals` +
        (errors.length
