...
//
// Dimensies die variëren tussen de 100 strategieën:
//   min_score, require_red, sector, max_pos, pos_size, hold_days,
//   stop_loss, take_profit, limit_buf, min_gold
//
// Gegroepeerd in 13 groepen (A–M) zodat per dimensie lessen getrokken
// kunnen worden.
...
interface Cfg {
  ...
  minScore: number;           // minimum goud_score (0 = uitgeschakeld)
  redReq: boolean;            // rood positief signaal verplicht
  sector: "all" | "biotech" | "mining";
  maxPos: number;             // max open posities tegelijk
  posSize: number;            // $ per positie
  holdDays: number;           // verplicht tijdvenster (dagen)
  stop: number | null;        // stop-loss fractie (0.15 = -15%), null = geen
  tp: number | null;          // take-profit fractie (0.50 = +50%), null = geen
  limitBuf: number | null;    // buy_limit * (1 + buf), null = geen limiet-check
  minGold: number;            // minimaal X gouden medailles vereist
}

// ── 100 strategieën ──────────────────────────────────────────────────────────
// Basis: score≥65, alle sectoren, 60d, stop-15%, geen TP, 8 pos, $1200, buf 10%, geen medaille
const B: Cfg = { slug:"s65",name:"Score ≥65 (basis)",grp:"A-Score", minScore:65,redReq:false,sector:"all",maxPos:8,posSize:1200,holdDays:60,stop:0.15,tp:null,limitBuf:0.10,minGold:0 };
...
// A:10 + B:6 + C:5 + D:4 + E:6 + F:8 + G:7 + H:5 + I:5 + J:8 + K:5 + L:5 + M:26 = 100

// ── Positieve signaal-types (zelfde als trade-background) ─────────────────────
...
async function run() {
  ...
  // 1. Ensure all 100 strategies + states exist (upsert by slug)
  const stratRows = STRATEGIES.map((s) => ({
    slug: s.slug, name: s.name, grp: s.grp,
    config: { minScore: s.minScore, redReq: s.redReq, sector: s.sector, maxPos: s.maxPos,
  ...
  // 2. Fetch shared market data (once for all strategies)
  const [statesRes, openRes, tickersRes, summaryRes, signalsRes] = await Promise.all([
    ...
    sb.from("xinix_strategy_positions").select("id, strategy_id, ticker, qty, avg_price, entry_date, scheduled_exit_date, stop_loss_price, take_profit_price").is("closed_at", null),
  ...
  for (const cfg of STRATEGIES) {
    ...
    for (const p of openPositions) {
      ...
      const stopHit = cfg.stop != null && price <= Number(p.avg_price) * (1 - cfg.stop);
      const tpHit   = cfg.tp   != null && price >= Number(p.avg_price) * (1 + cfg.tp);
      const timeUp  = now >= new Date(p.scheduled_exit_date);
      if (!stopHit && !tpHit && !timeUp) { stillOpenTickers.add(p.ticker); continue; }
      const proceeds = Number(p.qty) * price;
      const cost = Number(p.qty) * Number(p.avg_price);
      const retUsd = proceeds - cost;
      const retPct = cost > 0 ? (retUsd / cost) * 100 : 0;
      const holdDays = Math.max(0, Math.round((now.getTime() - new Date(p.entry_date).getTime()) / 86_400_000));
      const reason = stopHit ? `Stop-loss -${(cfg.stop! * 100).toFixed(0)}%`
                  : tpHit   ? `Take-profit +${(cfg.tp! * 100).toFixed(0)}%`
                             : `Tijdvenster ${cfg.holdDays}d verstreken`;
      exits.push({ id: p.id, data: { closed_at: now.toISOString(), closed_price: price, closed_reason: reason, return_usd: retUsd, return_pct: retPct, hold_days: holdDays } });
      cash += proceeds;
    }
    ...
    if (slotsAvailable > 0 && cash - cfg.posSize >= 200) {
      const candidates: Array<{ ticker: string; price: number; score: number; rankScore: number; sector: string | null; signals: SigRow[]; reason: string }> = [];
      ...
      for (const cand of candidates) {
        if (bought >= slotsAvailable || cash - cfg.posSize < 200) break;
        const qty = Math.floor((cfg.posSize / cand.price) * 1000) / 1000;
        if (qty <= 0) continue;
        const cost = qty * cand.price;
        if (cash - cost < 200) continue;
        const slp = cfg.stop != null ? Number((cand.price * (1 - cfg.stop)).toFixed(cand.price < 1 ? 4 : cand.price < 10 ? 3 : 2)) : null;
        const tpp = cfg.tp   != null ? Number((cand.price * (1 + cfg.tp)).toFixed(cand.price < 1 ? 4 : cand.price < 10 ? 3 : 2)) : null;
        ...
        buys.push({ strategy_id: sid, ticker: cand.ticker, qty, avg_price: cand.price,
    ...
    for (const p of (openByStrategy.get(sid) ?? [])) {
      if (!exits.some((e) => e.id === p.id)) {
        posVal += Number(p.qty) * (priceMap.get(p.ticker) ?? Number(p.avg_price));
      }
    }
  ...
  // Log run
  await sb.from("signal_runs").insert({ job: "xinix-sim", finished_at: now.toISOString(), ok: true,
    message: `${STRATEGIES.length} strategieën: ${exits.length} exits, ${buys.length} aankopen`,
    metrics: { strategies: STRATEGIES.length, exits: exits.length, buys: buys.length, equity_rows: equityRows.length } });

  return { ok: true, strategies: STRATEGIES.length, exits: exits.length, buys: buys.length };
}
