...
// Biotech press-release scanning — closes the #1 gap for 100%/day biotech
// events: positive Phase 2/3 readouts. Most baggers (VKTX +121%, KRYS +180%,
// AKRO +110%) come from a "topline data positive" or "primary endpoint met"
// PR, which is announced before the 8-K is filed and before clinicaltrials.gov
// updates the trial status.
...
// Patterns ordered by priority (first match wins per news item).
// Negative patterns guard against the "did NOT meet" false positives.
...
export default async () => {
  await logRun("poll-biotech-news", async () => {
      ...
      try {
        ...
        for (const item of news) {
          ...
          for (const p of BIOTECH_PATTERNS) {
            ...
            // Skip the topline_positive pattern when the item explicitly
            // negates the endpoint. trial_failed handles those separately.
...
export const config: Config = {
  schedule: "20 */2 * * *", // every 2h, offset from mining-news
};
